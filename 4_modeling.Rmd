...
pdat <- cbind(pdat1, predicted = p2$fit, se2 = p2$se.fit, observed, 
              deviance_residuals)
...
lpi_landuse_k_plots <- plot_grid(test_gam_plots[[1]],
                                 test_gam_plots[[2]],
                                 test_gam_plots[[3]],
                                 test_gam_plots[[4]],
                                 nrow = 2, align = "v")
...
indicator_mod_plots_k <- list()
indicator_mod_dfs_k <- list()
...
# Get predicted values for each time step
p2 <- predict.gam(indicator_mods[[j]], pdat1, 
                  se.fit = TRUE, response = TRUE)

# Get the deviance residuals
deviance_residuals <- residuals(indicator_mods[[j]], type = "deviance", 
                                response = TRUE)
...
#Get upper and lower bounds for predicted values (replace with bootstraps later)
df.res <- df.residual(indicator_mods[[j]])
crit.t <- qt(0.025, df.res, lower.tail = FALSE)
pdat <- transform(pdat,
                  upper_pred = predicted + (crit.t * se2),
                  lower_pred = predicted - (crit.t * se2))
...
# Calculate the derivatives and add that too
gratia_derivs_1 <- as.data.frame(derivatives(indicator_mods[[j]], order = 1, 
                   n = 290, interval = "confidence") ) 
...
#Make the plot
indicator_mod_plots_k[[j]] <- ggplot(data = indicator_mod_dfs_k[[j]]) +
            #geom_point(aes(x = annual_time_step, y = deviance_residuals), alpha = 0.2) +
            ...
            labs(y = indicator,
                 title = paste(scenario, indicator, "k =", kvals[[j]], sep = " ")) +
            theme_classic()
}

rli_landuse_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")
...
selected <- 2
selected_mods[[i]] <- indicator_mods[[selected]]
selected_mods_data[[i]] <- indicator_mod_dfs_k[[selected]]

```

## Harvested group

```{r land use harvested, warnings = FALSE }
...
harv_landuse_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

harv_landuse_k_plots
...
# Harvesting carnivores
## LPI

```{r carnivores lpi, warnings = FALSE }
...
lpi_carnivore_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

lpi_carnivore_k_plots
...
```{r carnivores rli, warnings = FALSE }
...
rli_carnivore_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

rli_carnivore_k_plots
...
```{r carnivores harvested, warnings = FALSE }
...
harv_carnivore_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

harv_carnivore_k_plots
...
lpi_herbivore_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

lpi_herbivore_k_plots
...
rli_herbivore_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

rli_herbivore_k_plots
...
harv_herbivore_k_plots <- plot_grid(indicator_mod_plots_k[[1]],
                                 indicator_mod_plots_k[[2]],
                                 indicator_mod_plots_k[[3]],
                                 indicator_mod_plots_k[[4]],
                                 nrow = 2, align = "v")

harv_herbivore_k_plots
...
# possibly some useful stuff https://fromthebottomoftheheap.net/2011/06/12/additive-modelling-and-the-hadcrut3v-global-mean-temperature-series/

i <- 4

kval <- 10

input_data <- scenario_indicator_single[[i]]

mod <- selected_mods[[i]]

mod_data <- selected_mods_data[[i]]
...
replicate[[k]] <- slice_sample(input_data, 
                          prop = 1, replace = TRUE) 
        

# Fit the model with the data
bootstrap_mods[[k]] <- gam(indicator_score ~ s(annual_time_step, k = kval),
          select = TRUE,
          method = "REML",
          data = replicate[[k]])
...
gratia_derivs_2 <- as.data.frame(derivatives(bootstrap_mods[[k]], order = 2, 
                   n = 290, interval = "confidence") ) %>% 
                   dplyr::select(derivative, lower, upper) %>% 
                   rename(second_deriv = derivative,
                          second_lower = lower,
                          second_upper = upper)
...
first_deriv_df <- do.call(rbind, bootstrap_mod_data) %>% 
             ...
             group_by(annual_time_step) %>% 
             arrange(first_deriv) %>% 
             summarise(first_lower = quantile(first_deriv, 
                                      probs = 0.025),
                       first_upper = quantile(first_deriv, 
                                      probs = 0.975)) %>% 
             merge(selected_mods_data[[i]][c("annual_time_step",
                                             "predicted",
                                               "derivative")],
                   by = "annual_time_step") %>% 
...
second_deriv_df <- do.call(rbind, bootstrap_mod_data) %>% 
             #Group by timestep
             group_by(annual_time_step) %>% 
             arrange(second_deriv) %>% 
             summarise(second_lower = quantile(second_deriv, 
                                      probs = 0.025),
                       second_upper = quantile(second_deriv, 
                                      probs = 0.975))


new <- selected_mods_data[[i]] %>% 
     dplyr::select(annual_time_step, obs, predicted, deviance_residuals, derivative) %>% 
...
ggplot(data = new) +
  geom_point(aes(x = annual_time_step, y = obs), alpha = 0.2) +
  geom_line(aes(x = annual_time_step, y = predicted)) +
  geom_ribbon(aes(x = annual_time_step, ymin = predicted_lower, ymax = predicted_upper),
              alpha = 0.3) +
  geom_line(aes(x = annual_time_step, y = positive_pred), col = "blue") +
  geom_line(aes(x = annual_time_step, y = negative_pred), col = "red") +
  labs(y = indicator,
