...
#' TODO: Figure out if we take top 3 strongest responses for each spp or just everything
#' above a threshold
...
replicate_plots <- list()
  ...
  m <- gam(std_abundance ~ s(annual_time_step, k = 10),
         select = TRUE,
         method = "GCV.Cp",
         data = species_data)

  replicate_data[[j]] <- get_species_gam_data(species_data, m, scenarios[[i]])

  replicate_plots[[j]] <- plot_test_gam(replicate_data[[j]])
  
  }

names(replicate_plots) <- group_ids

df <- do.call(rbind, replicate_data)

species_gam_data[[i]] <- df

species_gam_plots[[i]] <- replicate_plots
...
temp <- species_gam_data[[i]] %>% 
        merge(groups[c("group_id", "nutrition_source")], by.x = "indicator",
              by.y = "group_id") %>% 
        ...
        mutate(impact = ifelse(annual_time_step >= 95 &
               annual_time_step <= 105, "START",
               ifelse(annual_time_step >= 195 &
               annual_time_step <= 205, "STOP", NA))) 

# Split by positive or negative trends

influential_species_data[[i]] <- temp %>% 
       dplyr::select(-annual_time_step) %>% # Remove time steps
       mutate(absolute_fd = abs(first_derivative)) %>%  # convert all first derivatives to absolute value
       group_by(impact, indicator) %>% # For each start stop event, for each species
       filter(absolute_fd == max(absolute_fd)) %>% # Get the max & min value
       ungroup(.) %>% # Release event/spp grouping
       group_by(impact) %>% # Group by event alone
       arrange(desc(absolute_fd)) %>%  # arrange species by highest impact
       slice(1:3) # Get the three species with the strongest response to each event

fgs <- unique(influential_species_data[[i]]$indicator)

influential_species_plots[[i]] <- species_gam_plots[[i]][fgs]
