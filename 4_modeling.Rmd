...
observed <- data$indicator_score_std
...
influential_species_data_start <- list()
influential_species_data_stop <- list()
fgs <- list()
...
start <- temp4[1:n,] # Get n most influential spp
...
stop <- temp4[1:n,] # Get n most influential spp
...
influential_spp_plots[[i]] <- ggplot(temp) +
        geom_line(aes(annual_time_step, predicted, col = nutrition_source)) +
        geom_line(aes(x = annual_time_step, y = positive_trend,
        ...
        geom_ribbon(aes(x = annual_time_step, 
                  ...
                  labs(x = "Time (years)",
                       ...
                       title = paste(scenarios[[i]], reason, sep = " ")) + 
        geom_vline(xintercept=c(100,200), linetype = "dotted") +
...
m <- gam(indicator_score_std ~ s(annual_time_step, k = kvals[1]),
         select = TRUE,
         method = ind_method,
         data = scenario_indicator_single[[i]])

m1 <- gam(indicator_score_std ~ s(annual_time_step, k = kvals[[2]]),
          select = TRUE,
          method = ind_method,
          data = scenario_indicator_single[[i]])

m2 <- gam(indicator_score_std ~ s(annual_time_step, k = kvals[[3]]),
          select = TRUE,
          method = ind_method,
          data = scenario_indicator_single[[i]])
...
```{r plot bootstrapped models in grid}

names(bootstrapped_plots) <- names(scenario_indicator_single)

bs_plots_list <- list(bootstrapped_plots[1:3],
             bootstrapped_plots[5:8],
             bootstrapped_plots[9:12])

aggregated_plots <- list()

for(i in seq_along(bs_plots_list)){
  
  title <- ggdraw() + draw_label(scenarios[[i]])

  aggregated_plots[[i]] <- plot_grid(title,
                                     bs_plots_list[[i]][[1]],
                                     bs_plots_list[[i]][[2]],
                                     bs_plots_list[[i]][[3]],
                                     bs_plots_list[[i]][[4]],
  ...
  ggsave(file.path(manuscript_plots_folder, 
                   paste(scenarios[[i]], "bt.png", 
                         sep = "_")),
       width = 10,
       height = 5,
       units = "cm",
