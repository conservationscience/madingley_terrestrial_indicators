...
trend_data[[j]] <- list_of_model_data[[j]] %>% 
                   #Select the significant positive and negative trends
...
all_trends[[i]] <- robust_trends[[i]] %>% 
     ...
     filter(annual_time_step >= 95 &
            ...
            annual_time_step <= 205) %>% 
     filter(k_value == 10) %>% 
     mutate(plotted_derivative = ifelse(is.na(positive_across_all_models), 0,
                                      ifelse(is.na(negative_across_all_models), 0,
                                    first_derivative)),
            significant = ifelse(positive_across_all_models == 1, TRUE,
                         ifelse(negative_across_all_models == 1, TRUE,
                                    FALSE)),
            impact = ifelse(annual_time_step >= 95 &
                            annual_time_step <= 105, "START", "STOP")) #%>% 
     # dplyr::select(indicator, scenario, impact, signficant,
     #               first_derivative, 
     #               plotted_derivative)

}

trend_table <- do.call(rbind, all_trends) %>% 
               filter(scenario != "000_Baseline") %>% 
               mutate(first_derivative = round(first_derivative, 3)) %>% 
               group_by(scenario, indicator, impact) %>% 
               mutate(ave_trend = mean(first_derivative, na.rm = TRUE),
                      ave_trend = ifelse(significant == TRUE, ave_trend, 0)) %>% 
               slice(1) 
...
heatmap <- ggplot(plot_data, aes(x = label, y = indicator, 
                                 fill = ave_trend)) +
