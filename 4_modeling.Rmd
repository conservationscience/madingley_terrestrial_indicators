...
plot_bootstrapped_model <- function(data){
...
boot_plot <- ggplot(data = data) +
    geom_rect(aes(xmin = 95, xmax = 105,
                ymin = -Inf, ymax = Inf), col = "grey60", alpha = 0.5) +
...
compare_trends(test_gam_data)

lpi_land_use_trend <- compare_trends(test_gam_data)
...
```{r land use rli, warnings = FALSE, message = FALSE}
...
rli_landuse_trend <- compare_trends(test_gam_data)
...
```{r havrested rli, warnings = FALSE, message = FALSE}
...
harvested_landuse_trend <- compare_trends(test_gam_data)
...
lpi_carnivore_trend <- compare_trends(test_gam_data)
...
rli_carnivore_trend <- compare_trends(test_gam_data)
...
harvested_carnivore_trend <- compare_trends(test_gam_data)
...
lpi_herbivore_trend <- compare_trends(test_gam_data)
...
rli_herbivore_trend <- compare_trends(test_gam_data)
...
harvested_herbivore_trend <- compare_trends(test_gam_data)
