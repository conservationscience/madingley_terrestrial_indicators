...
model <- selected_mods[[i]]

model_data <- selected_mods_data[[i]]
...
first_deriv_df <- do.call(rbind, bootstrap_mod_data) %>% 
             ...
             mutate(significant = ifelse(first_lower < 0 & first_upper > 0, 
                    ...
                    negative_trend = ifelse(significant == TRUE & first_derivative < 0,
                                            predicted, NA),
                    positive_der = ifelse(significant == TRUE & first_derivative > 0,
                                            first_derivative,NA),
                    negative_der = ifelse(significant == TRUE & first_derivative < 0,
                                            first_derivative, NA)) %>% 
...
second_deriv_df <- do.call(rbind, bootstrap_mod_data) %>% 
             ...
             summarise(second_derivative = second_derivative,
                       second_lower = quantile(second_derivative, 
                                      probs = 0.025),
...
new <- model_data %>% 
     ...
     merge(second_deriv_df, by ="annual_time_step") %>% 
             mutate(significant_2 = ifelse(second_lower < 0 & second_upper > 0, 
                                         FALSE, TRUE),
                    positive_trend = ifelse(significant_2 == TRUE & second_derivative > 0,
                                            predicted,NA),
                    negative_trend = ifelse(significant_2 == TRUE & second_derivative < 0,
                                            predicted, NA),
                    positive_der = ifelse(significant_2 == TRUE & second_derivative > 0,
                                            second_derivative,NA),
                    negative_der = ifelse(significant_2 == TRUE & second_derivative < 0,
                                            second_derivative, NA))
...
boot_plot <- ggplot(data = data) +
            ...
            #geom_line(aes(x = annual_time_step, y = positive_trend), col = "blue") +
            #geom_line(aes(x = annual_time_step, y = negative_trend), col = "red") +
            labs(y = data$indicator[1],
                           title = paste(data$scenario[1], 
                                         data$indicator[1], "k =", 
                                         data$kvalue[1], sep = " ")) +
                      theme_classic()
...
test <- bootstrap_selected_model(scenario_indicator_single[[i]],
                                 selected_mods[[i]], 
                                 selected_mods_data[[i]], 100)

plot_bootstrapped_model(test)
