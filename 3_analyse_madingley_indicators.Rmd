...
#' @param bintype a string representing how you want to bin the rates of change,
#' can be either Available are: "interval" (equal interval width), "frequency" 
#' (equal frequency), "cluster" (k-means clustering) and "fixed" 
#' (categories specifies interval boundaries). Note that equal frequency does 
#' not achieve perfect equally sized groups if the data contains duplicated values.
#' @return a dataframe of the proportion of species in each rate of change bin
#' per timestep

bin_rates_of_change <- function(data, bintype){
  ...
  lpi_inputs <- filtered_inputs %>%
    ...
    ungroup(.) %>%
    # mutate(dt_bins = ifelse(dt < 0.5, "<0.5",
    #                  ifelse(dt > 0.4 & dt < 0.75, " 0.5 - 0.74",
    #                  ifelse(dt > 0.74 & dt < 1, "0.75 - 0.99",
    #                  ifelse(dt > 0.9999 & dt < 1.5,"1 - 1.4",
    #                  ifelse(dt > 1.4, " > 1.4", NA)))))) %>% 
    mutate(dt_bins = discretize(dt, bintype, 5)) %>% 
    group_by(annual_time_step) %>% 
    mutate(num_groups = n_distinct(group_id)) %>% 
    group_by(annual_time_step, dt_bins) %>% 
    filter(!is.na(dt_bins)) %>% 
    summarise(prop_declining = n_distinct(group_id)/num_groups) %>% 
    distinct(.)
...
indicator_plots[[j]] <- ggplot() +
  ...
  geom_vline(xintercept = 200, col = "blue")
...
set_breaks <- c(-Inf, -0.5, -0.02, -0.01, 0.01, 0.02, 0.5,Inf)
...
bin_rates_of_change <- function(data, bintype){
  ...
  lpi_inputs <- filtered_inputs %>%
    ...
    mutate(dt = log10(current_abundance/previous_abundance)) %>% # rate of change
    ungroup(.) %>%
    # mutate(dt_bins = as.factor(ifelse(dt <= -0.5, " < -0.5",
    #                  ifelse(dt > -0.5 & dt <= -0.02, "-0.5 to -0.02",
    #                  ifelse(dt > -0.02 & dt <= -0.01, "-0.02 to -0.01",
    #                  ifelse(dt > -0.01 & dt <= 0.01, "-0.01 to 0.01",
    #                  ifelse(dt > 0.01 & dt <= 0.02, "0.01 to 0.02",
    #                  ifelse(dt > 0.02 & dt <= 0.5, "0.02 to 0.5",
    #                  ifelse(dt > 0.5,"> 0.05", NA)))))))),
    #        ordered = TRUE) %>%
    mutate(dt_bins = discretize(dt, "fixed", 
                                breaks =  c(-2.5, -0.5, -0.02, -0.01, 
                                            0.01, 0.02, 0.5,2.5),
                                ordered_result = TRUE),
           decline_fac = as.character(dt_bins)) %>% 
    group_by(annual_time_step) %>% 
    mutate(num_groups = n_distinct(group_id)) %>% 
    group_by(annual_time_step, dt_bins) %>% 
    filter(!is.na(dt_bins)) %>% 
...
# decline_fac <- factor(lpi_inputs$dt_bins,
#       levels = c(" < -0.5", "-0.5 to -0.02", "-0.02 to -0.01", "-0.01 to 0.01",
#                  "0.01 to 0.02", "0.02 to 0.5", "> 0.5"))
#   
# lpi_inputs <- cbind(lpi_inputs,
#   decline_fac = decline_fac)
...
rate_change_data <- bin_rates_of_change(scenario_redlist_data_sampled[[i]],
                                        "fixed") %>% 
                                        mutate(scenario = scenarios[[i]])
  
rate_of_change_plots[[i]] <- ggplot(rate_change_data,
                                    aes(x = annual_time_step, 
                                        y = prop_declining, 
                                        fill = dt_bins)) +
    geom_area(alpha = 0.4) +
   ...
   labs(title = scenarios_title[[i]],
        x = "Time (years)",
        y = "Proportion of functional groups in decline category") +
...
ggsave(file.path(analysis_plots_folder, "baseline_spp_density.pdf"),
...
ggsave(file.path(analysis_plots_folder, "land_use_spp_density.pdf"),
...
ggsave(file.path(analysis_plots_folder, "carnivore_spp_density.pdf"),
...
ggsave(file.path(analysis_plots_folder, "herbivore_spp_density.pdf"),
