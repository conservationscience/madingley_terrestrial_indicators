...
bin_rates_of_change <- function(data, set_breaks){
  ...
  dt_bins <- lpi_inputs %>% 
    ...
    summarise(group_id = group_id,
              ...
              prop_declining = n_distinct(group_id)/num_groups) %>% 
    distinct(.)
...
set_breaks <- c(-3,-0.0894307914, -0.0166148404, -0.0003499485,
...
rate_of_change_plots[[i]] <- ggplot(plot_data,
  ...
  ggsave(file.path(manuscript_plots_folder, paste(scenario,
                                                "decline_rate_stacked_bar_plots.png",
                                                sep = "_")),
       width = 14,
       height = 9,
       units = "cm",
...
scenario_important_abundance[[i]] <-  subset_data %>% 
      merge(important_list[[i]][c("group_id", "reason")], by = "group_id") %>% 
...
test <- scenario_important_abundance[[2]]

saveRDS(scenario_important_abundance, file.path(analysis_inputs, input_date, "scenario_important_group_abundance.rds"))

```

# Plot red list status of important groups

```{r rli status plots}

rl_status_change_plots <- list()

for (i in seq_along(scenario_important_abundance)) {
  
data <- scenario_important_abundance[[i]] %>% 
        filter(indicator == "harvested group")
...
if(scenario_important_abundance[[i]]$scenario[1] == "000_Baseline") {
  
  rl_status_change_plots[[i]] <- ggplot(data2, aes(annual_time_step,indicator_score,
...
} else if(scenario_important_abundance[[i]]$scenario[1] == "100_Land_use") {
  
  rl_status_change_plots[[i]] <- NULL
...
rl_status_change_plots[[1]]
rl_status_change_plots[[2]]
rl_status_change_plots[[3]]
rl_status_change_plots[[4]]
