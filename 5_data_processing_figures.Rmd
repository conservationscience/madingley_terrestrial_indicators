...
ggsave(file.path(supporting_info_plots_folder, "0_raw_abundance.png"),
       width = 20,
       height = 8,
       units = "cm",
       abundance_plot)
...
averaged$abundance[6] <- 0

averaged$abundance[18] <- 0
...
averaged$abundance[6] <- NA

averaged$abundance[18] <- NA

averaged <- averaged %>% 
            filter(!is.na(abundance))
  
averaged_plot2 <- ggplot() +
                 geom_path(data = averaged, aes(annual_time_step, abundance)) +
                geom_point(data = averaged, aes(annual_time_step, abundance),
  ...
  labs(x = "Time",
       y = "Abundance",
       title = "False extinction removed") +
  theme(axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + 
...
ggsave(file.path(supporting_info_plots_folder, "4_false_ex_removed.png"),
       width = 8,
       height = 8,
       units = "cm",
       averaged_plot2)
...
smoothed$abundance[6] <- NA

smoothed$abundance[18] <- NA
...
ggsave(file.path(supporting_info_plots_folder, "5_false_ex_removed.png"),
       width = 8,
       height = 8,
       units = "cm",
       smoothed_plot)
